...
public class Robot extends TimedRobot {
  ...
  private boolean startLedState = false;
  private boolean enabledByButton = false;
  ...
  public void robotPeriodic() {
    ...
    if (!RobotContainer.drive.getStartBtn() && !active)
    {
        ds.enable(); // enable the robot.
        active = true;
    }
    if (!RobotContainer.drive.getStopBtn() && active)
    {

        ds.disable(); // disable the robot.
        active = false;
    }
   // System.out.println(startButton.get());
  }
  ...
  public void disabledInit() {
    RobotContainer.drive.setStarLed(false);
  ...
  public void disabledPeriodic() {
    RobotContainer.drive.setStartedBtnLed(false);
    RobotContainer.drive.setStopBtnLed(false);
  }
  ...
  public void autonomousPeriodic() {
    if (!RobotContainer.drive.getStopBtnLed()){
      RobotContainer.drive.setStarLed(true);
      } else {
        RobotContainer.drive.setStarLed(false);
      }
      double now = Timer.getFPGATimestamp();
      if (now - lastBlinkTime >= 0.5 && !RobotContainer.drive.getStopBtnLed()) {   // 1 секунда
          startLedState = !startLedState; // инверсия true/false
          RobotContainer.drive.setStartedBtnLed(startLedState);
          lastBlinkTime = now;
      }
  }
